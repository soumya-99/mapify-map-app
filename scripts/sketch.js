...
function draw() {
	...
	for (let i = 0; i < grid.length; i++) {
		for (let j = 0; j < grid[i].length; j++) {
			...
			else if (grid[i][j].isSource === true) fill(0, 255, 0)
			else if (grid[i][j].isDest === true) fill(0, 0, 255)
...
function init() {
	...
	for (let i = 0; i < n; i++) {
		...
		for (let j = 0; j < n; j++) {
			...
			if (colour === colorBlack) tempOb.toggle() // basically make it wall
...
function keyPressed() {
	...
	if (keyCode === 13) {
		...
		for (let k = 0; k < length; k++)
			for (let i = 0; i < n; i++)
				for (let j = 0; j < n; j++)
					if (grid[i][j].vertexNumber === path[k]) grid[i][j].inPath()
	}
}
